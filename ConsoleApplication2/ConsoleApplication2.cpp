...
/*
請以 C++ 或 Java 實作以下副程式 MilitaryTimeToRegularTime( )，
將24小時制的時間 (稱為 military time)，轉換為12小時制的時間，並回傳之。
請注意，輸入參數24小時制時間為字串 (string)，長度固定為5個字元(char)，
若有格式錯誤，例如長度不是5、不含冒號、不是數字字元或時間不合理等，
則此副程式應回傳 error 字串。
